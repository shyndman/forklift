...
The agent container SHALL mount only the run's `workspace`, `harness-state`, and `control` directories, mapped to `/workspace`, `/harness-state`, and `/forklift-control` respectively, exposing no Git remotes, SSH keys, or host credentials. The `control` mount exists solely for host-owned runtime sockets such as `FORKLIFT_REBASE_EVENTS_SOCK=/forklift-control/rebase-events.sock`; it MUST NOT be repurposed as a general artifact directory. Within the container the workspace MUST behave as a regular Git repository but without any configured remotes, and the container SHALL allow outbound network access solely for dependency downloads.

#### Scenario: Remote-free workspace
- **WHEN** the agent inspects `git remote -v` inside `/workspace`
- **THEN** it observes no configured remotes and cannot push to any external repository, while still being able to read/write files inside the workspace and connect to the host-provided control socket when `FORKLIFT_REBASE_EVENTS_SOCK` is set
...
#### Scenario: Paused rebase events stream without replacing existing logs
- **WHEN** the harness pauses on a conflict or advances a paused rebase through continue, skip, auto-skip, completion, or abort
- **THEN** it emits best-effort structured events over `FORKLIFT_REBASE_EVENTS_SOCK` so the host can render live `Rebase N/Total` or `Conflict N/Total` lines
- **AND** `/harness-state/opencode-client.log`, `/harness-state/harness-status.txt`, and top-level container stdout/stderr keep their existing responsibilities
