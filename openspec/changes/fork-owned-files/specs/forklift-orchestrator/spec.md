...
#### Scenario: Files command stays outside orchestration lifecycle
- **WHEN** the user runs `forklift files` for any supported main-branch value
- **THEN** the command performs local branch analysis without creating a run directory
- **AND** no container launch or run-state lifecycle file updates occur
- **AND** no local publication branch is created as part of files command execution
