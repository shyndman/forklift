...
- **Testing**: Need a manual smoke test documented in README verifying that running `uv run forklift --verbose --model claude-35-sonnet` produces populated `/harness-state/opencode-{server,client}.log` files and that removing a required env key causes the CLI to abort before container launch.
